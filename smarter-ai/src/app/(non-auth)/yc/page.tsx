...
const AiMeetingContent: React.FC<{ meetingId: string }> = ({ meetingId }) => {
  ...
  const handleStartStopClick = async () => {
    if (isSessionActive) {
      ...
      try {
        ...
        console.log(
          "Original system prompt from job data:",
          systemPrompt.substring(0, 100) + "...",
        )

        // For demo mode, we'll bypass the validation checks
        ...
        const demoData = {
          ...
          candidateName: candidateData?.name || "Demo User",
        ...
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{jobTitle\}\}/g, demoData.jobTitle)
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{candidateName\}\}/g, demoData.candidateName)
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{requirements\}\}/g, demoData.requirements)
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{responsibilities\}\}/g, demoData.responsibilities)
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{desirables\}\}/g, demoData.desirables)
        fullSystemPrompt = fullSystemPrompt.replace(/\{\{cv\}\}/g, demoData.combinedCvContent)
  ...
  const fetchConnectionDetails = async (systemPrompt: string) => {
    try {
      ...
      if (candidateData) {
        contextData.candidateName = candidateData.name || "Y Combinator"
      }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6">
            ...
            <div className="mt-3 mb-3 p-2 border rounded-md bg-gray-50 text-sm">
              ...
              <p className="text-xs text-gray-600">YC</p>
            </div>
