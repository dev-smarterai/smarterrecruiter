...
class WebSocketHandler {
    static async create(config) {
        ...
        try {
            // Handle mock URL case for development
            if (config.signedUrl === "mock-signed-url") {
                console.log("Using mock WebSocket for development");
                
                // Create a mock WebSocket that doesn't really connect
                const mockSocket = {
                    addEventListener: () => {},
                    send: () => {},
                    close: () => {}
                ...
                // Return a mock handler with minimal functionality
                return {
                    socket: mockSocket,
...
class ConversationManager {
    static async startSession(config: ConversationConfig) {
        ...
        // Check if we're using a mock URL (development mode)
        const isDevelopmentMode = config.signedUrl === "mock-signed-url";
        ...
        try {
            // Use real connections unless in development mode
            if (!isDevelopmentMode) {
                ...
                // In development mode, just get the mock WebSocket connection
                console.log("Using development mode with mock resources");
                wsConnection = await WebSocketHandler.create(config);
                
                // Create minimal mock for other components
                audioInput = { 
                    processorNode: { port: { onmessage: null } },
                    close: async () => {} 
                };
                audioOutput = { 
                    processorNode: { port: { onmessage: null, postMessage: () => {} } },
                    gainNode: { gain: { value: 0 } },
                    close: async () => {} 
                };
            }

            const manager = new ConversationManager(settings, wsConnection, audioInput, audioOutput);
            
            // If in development mode, simulate a response after a delay
            if (isDevelopmentMode) {
                setTimeout(() => {
                    settings.onMessage({ 
                        source: 'ai', 
                        message: 'This is a mock response in development mode. The real ElevenLabs integration requires API keys.' 
                    });
                }, 1000);
            }
    ...
    constructor(settings, wsConnection, audioInput, audioOutput) {
        ...
        const isMockConnection = wsConnection.conversationId === "mock-conversation-id";

        // Initialize event listeners only for real connections
        if (!isMockConnection) {
            this.initializeEventListeners();
        } else {
            // For mock connections, just update status
            console.log("Using mock connection - skipping event listeners");
            
            // Notify about mock connection
