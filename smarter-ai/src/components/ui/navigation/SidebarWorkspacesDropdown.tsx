...
type WorkspacesDropdownProps = {
  iconOnly?: boolean;
}

export const WorkspacesDropdownDesktop = ({ iconOnly = false }: WorkspacesDropdownProps) => {
    ...
    <>
      ...
      >
        <DropdownMenuTrigger asChild>
          <button
            className={cx(
              "flex items-center rounded-md border border-gray-300 bg-white shadow-sm transition-all hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-950 hover:dark:bg-gray-900",
              iconOnly ? "w-auto p-1.5 gap-0" : "w-full p-2 gap-x-2.5",
              focusInput,
            )}
          >
            <span
              className={cx(
                "flex items-center justify-center rounded bg-indigo-600 text-xs font-medium text-white dark:bg-indigo-500",
                iconOnly ? "size-7 p-1.5" : "aspect-square size-8 p-2"
              )}
              aria-hidden="true"
            >
              RA
