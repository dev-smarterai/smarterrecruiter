...
export default function useWebRTCAudioSession(
  ...
  // Add state for the audio element
  ...
  function configureDataChannel(dataChannel: RTCDataChannel) {
    ...
    // No language preference message is sent here - we rely entirely on the system prompt
    // set on the window object which is sent during token acquisition
  }
  ...
  async function getEphemeralToken() {
    try {
      // Check if we have a custom system prompt in the window object (for AI meetings)
      let systemPrompt;
      
      console.log("Starting getEphemeralToken, checking for system prompts...");
      
      // Debug all possible sources of system prompts
      if (typeof window !== 'undefined') {
        console.log("systemPromptForAIMeeting exists:", !!(window as any).systemPromptForAIMeeting);
        console.log("languagePrompt exists:", !!(window as any).languagePrompt);
      }
      
      if (typeof window !== 'undefined' && (window as any).systemPromptForAIMeeting) {
        systemPrompt = (window as any).systemPromptForAIMeeting;
        
        // Validate that the system prompt is meaningful - at least 100 characters
        ...
        console.log("===== DETECTED CUSTOM SYSTEM PROMPT FROM WINDOW OBJECT =====");
        console.log("System prompt length:", systemPrompt.length);
        console.log("FULL SYSTEM PROMPT:");
        console.log(systemPrompt);
        console.log("==========================================================");
        
        // Explicitly check if languagePrompt exists and warn about potential override
        if ((window as any).languagePrompt) {
          console.warn("WARNING: Both systemPromptForAIMeeting and languagePrompt exist in window object.");
          console.warn("Ensuring systemPromptForAIMeeting takes precedence, but there's risk of conflict.");
          // Force languagePrompt to be undefined to prevent any chance of it being used
      ...
      console.log("Sent system prompt to /api/session endpoint");

      if (!response.ok) {
        const responseText = await response.text();
        throw new Error(`Failed to get ephemeral token: ${response.status}. Response: ${responseText}`);
      }
  ...
  function toggleMute() {
    ...
    setIsMuted(prevMuted => {
      ...
      // Get all audio tracks from the current stream and toggle their enabled state
  ...
  async function startSession() {
    try {
      ...
      dataChannel.onopen = () => {
        console.log("Data channel open, state:", dataChannel.readyState);
